...
export default function InputAbsenGuru({
  user,
  onLogout,
  currentPage,
  onMenuClick,
  ...
  const handleBatal = () => {
    if (window.confirm('Apakah Anda yakin ingin membatalkan? Perubahan tidak akan disimpan.')) {
      setSiswaList(siswaList.map((s) => ({ ...s, status: null })));
    }
  };
              ...
              <tbody>
                {siswaList.map((siswa, idx) => (
                  <tr
                    ...
                    onMouseLeave={(e) => {
                      (e.currentTarget as HTMLTableRowElement).style.backgroundColor = idx % 2 === 0 ? '#FFFFFF' : '#F9FAFB';
                    }}
                  >
                    <td style={{ padding: '16px', fontSize: '14px', color: '#111827', fontWeight: '500' }}>
                      {idx + 1}
                    </td>
                    <td style={{ padding: '16px', fontSize: '14px', color: '#374151', fontWeight: '400' }}>
                      {siswa.nisn}
                    </td>
                    <td style={{ padding: '16px', fontSize: '14px', color: '#111827', fontWeight: '500' }}>
                      {siswa.nama}
